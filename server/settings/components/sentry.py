...
if SENTRY_DSN:  # noqa: C901
    ...
    # Проброс номера сборки
    if build_id := config('NOVA_BUILD_ID', cast=str, default=''):
        if not build_id.lower().startswith('no'):
            sentry_sdk.set_tag('build', build_id)

    # Проброс версии релиза
    if release := config('NOVA_RELEASE', cast=str, default=''):
        if not release.lower().startswith('no'):
            extra_params['release'] = release
